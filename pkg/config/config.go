...
func Load() *Config {
	...
	flag.StringVar(&cfg.ClientID, "client-id", "", "OAuth 2.0 Client ID (required for initial authentication)")
	flag.StringVar(&cfg.ClientSecret, "client-secret", "", "OAuth 2.0 Client Secret (required for initial authentication)")
	...
	// Token path with default
	tokenPath := os.Getenv("HOME")
	if tokenPath == "" {
		tokenPath = "/root"
	}
	defaultTokenPath := filepath.Join(tokenPath, ".tado-exporter", "token.json")
	flag.StringVar(&cfg.TokenPath, "token-path", defaultTokenPath, "Path to token file for persistent storage")
